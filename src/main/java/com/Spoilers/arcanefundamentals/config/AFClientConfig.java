...
public class AFClientConfig {
    ...
    public static void init(ForgeConfigSpec.Builder clientConfig) {
        ...
        ELEMENT_SIZE = clientConfig.comment("Size of the wellspring display").defineInRange("display_size", 0.4, 0.0, 8.0);
        ELEMENT_HEIGHT = clientConfig.comment("Height of the wellspring display in blocks, relative to player head").defineInRange("display_height", 0, -8.0, 8.0);
        ELEMENT_OFFSET = clientConfig.comment("Horizontal offset of the wellspring display in blocks, negative values move right").defineInRange("display_offset", 0.0, -8.0, 8.0);
        clientConfig.pop();
    }
}
